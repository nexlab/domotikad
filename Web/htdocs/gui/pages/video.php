...
if($GUISUBSECTION!="" && is_numeric($GUISUBSECTION))
{
   ...
   if(is_array($v) && count($v)>0) {
      ...
      foreach($v as $cam) {
         ...
         $panels[]=array('panel_title'=>$cam['button_name'],'panel_position'=>$pos,'panel_content'=>$cam['videostream'],
...
if(!is_array($panels) or count($panels)<1) {
   ...
   if(is_array($v) && count($v)>0) {
      $pos=1;
      foreach($v as $cam) {
         $DEFPANELS[]=array('panel_title'=>$cam['button_name'],'panel_position'=>$pos,'panel_content'=>$cam['videostream'],
                            'panel_height'=>'100%','panel_type'=>'cameras')+$PANELDEFAULTS;
         $pos++;
      }
   }
   ...
   foreach($panels as $p) {
      $q="INSERT INTO user_gui_panels 
         ...
         VALUES
         ('".$_DOMOTIKA['username']."','video','".$p['panel_title']."','".$p['panel_type']."',
